...
> **相关文档**：[下载与安装教程.md](下载与安装教程.md) · [无线调试配对失败怎么办.md](无线调试配对失败怎么办.md) · [连接失败排查方法.md](连接失败排查方法.md) · [互传联盟是什么意思.md](互传联盟是什么意思.md)
...
2. 启动 Shizuku。你的系统是 Android 11 及以上，可以用**无线调试**方式启动，全程不用电脑：
   - 开发者选项：「设置 → 关于手机」里连续点「版本号」7 次开启；
   - 进「开发者选项」，打开 **USB 调试**，再打开 **无线调试**（手机需连着 WiFi）；
   - 在 Shizuku 里点「配对」，到系统「无线调试」页点「使用配对码配对设备」，把 6 位配对码填进 **Shizuku 的通知**——配对只需做一次；
   - 回 Shizuku 点「启动」，看到已运行即可。
   系统低于 Android 11 则需要连电脑用 adb 启动，命令以 Shizuku 界面给出的为准。
...
- **Shizuku 必须保持运行**。手机重启后 Shizuku 会停，要重新点一次「启动」（配对关系还在，不用重输配对码），CatShare 才能继续自动获取。
- 配对环节出问题（一直搜索配对服务、配对码填了就报错、启动不起来）单独有一篇细讲，见 [无线调试配对失败怎么办.md](无线调试配对失败怎么办.md)。
